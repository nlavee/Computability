...
ggplot(KnapsackRes) + 
  geom_point(aes(x=as.factor(count), y=DP1.Running.Time, colour = "DP1.Running.Time")) + 
  geom_point(aes(x=as.factor(count), y=DP2.Running.Time, colour = "DP2.Running.Time")) + 
  geom_point(aes(x=as.factor(count), y=Greedy.Running.Time, colour = "Greedy.Running.Time")) + 
  geom_point(aes(x=as.factor(count), y=FPTAS.Running.Time, colour = "FPTAS.Running.Time")) +
  xlab("Instances") + 
  ylab("RunningTime")
